...
from custom_components.ecoflow_cloud_claudiosv.devices import BaseDevice, EcoflowDeviceUpdateCoordinator
...
class EcoFlowBaseCommandEntity(EcoFlowDictEntity):
    ...
    def send_set_message(self, target_value: Any, command: dict):
        # Route through the API client's send_set_message (not mqtt_client's),
        # since it accepts either a plain dict or a protobuf Message object
        # (command_dict() returns Message instances for protobuf-based
        # devices like River3/Wave3/PowerStream); mqtt_client's is dict-only.
